...
fp1 = open("C:/Users/viksp/Documents/Folder_of_Folders/Polygence_code/calling-out-bluff-models_test/Model3(SkipFlow)/glove/glove.6B.300d.txt", "r")
glove_emb = {}
with open("C:/Users/viksp/Documents/Folder_of_Folders/Polygence_code/calling-out-bluff-models_test/Model3(SkipFlow)/glove/glove.6B.300d.txt", "r",errors="replace") as f:
    for line in f:
        temp = line.split(" ")
...
class Neural_Tensor_layer(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        result = K.tanh(K.reshape(K.concatenate(
            bilinear_tensor_products, axis=0), (batch_size, k))+feed_forward)
    ...
    def build(self, input_shape):
        ...
        k = self.output_dim
        d = self.input_dim
        W_val = stats.truncnorm.rvs(-2 * std, 2 *
                                    std, loc=mean, scale=std, size=(k, d, d))
        V_val = stats.truncnorm.rvs(-2 * std, 2 *
                                    std, loc=mean, scale=std, size=(2*d, k))
...
def SKIPFLOW(embedding_matrix, vocab_size, lstm_dim=50, lr=1e-4, lr_decay=1e-6, k=5, eta=3, delta=50, activation="relu", maxlen=MAX_SEQUENCE_LENGTH, seed=None):
    ...
    pairs = [((eta + i * delta) % maxlen, (eta + i * delta + delta) % maxlen)
             for i in range(maxlen // delta)]
    hidden_pairs = [(Lambda(lambda t: t[:, p[0], :])(side_hidden_states), Lambda(
        lambda t: t[:, p[1], :])(side_hidden_states)) for p in pairs]
    ...
    dense = Dense(256, activation=activation,
                  kernel_initializer=initializers.glorot_normal(seed=seed))(co_tm)
    dense = Dense(128, activation=activation,
                  kernel_initializer=initializers.glorot_normal(seed=seed))(dense)
...
def init(essay_type='4'):
    ...
    fp = open("C:\\Users\\viksp\\Documents\\Folder_of_Folders\\Polygence_code\\calling-out-bluff-models_test\\Model3(SkipFlow)\\data\\training_set_rel3.tsv",
    ...
    sf_1 = SKIPFLOW(embedding_matrix, vocab_size, lstm_dim=50, lr=2e-4,
                    lr_decay=2e-6, k=4, eta=13, delta=50, activation="relu", seed=None)

    sf_1.load_weights(
        "/Users/saiteja/playground/IRE/major_project/github/webapp/models/weights/" + str(essay_type) + "_weights.h5")
