...
for essay in essayColumn:
    essay=essay[0]
    sentences = split_into_sentences(essay)

    whole_essay = " ".join(sentences[0:len(sentences)//2+1]) + " ".join(sentences[len(sentences)//2+1:])
...
for essay in essayColumn:
    essay=list(temp_essays[i])
    i+=1

df.to_csv('dev_2nd_method_set_1.tsv', sep="\t",index=False)
