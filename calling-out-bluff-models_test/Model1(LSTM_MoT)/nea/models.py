...
def create_model(args, initial_mean_value, overal_maxlen, vocab):
	...
	if args.recurrent_unit == 'lstm':
		from tensorflow.python.keras.layers.recurrent import LSTM as RNN
	elif args.recurrent_unit == 'gru':
		from tensorflow.python.keras.layers.recurrent import GRU as RNN
	elif args.recurrent_unit == 'simple':
		from tensorflow.python.keras.layers.recurrent import SimpleRNN as RNN
	...
	if args.model_type == 'cls':
		...
		if args.rnn_dim > 0:
			model.add(RNN(args.rnn_dim,Dropout(dropout_W), return_sequences=False,reccurent_dropout=dropout_U))
		...
		if args.cnn_dim > 0:
			model.add(Conv1DWithMasking(nb_filter=args.cnn_dim, filter_length=args.cnn_window_size, border_mode=cnn_border_mode, subsample_length=1))
		if args.rnn_dim > 0:
			model.add(RNN(args.rnn_dim, Dropout(dropout_W), recurrent_dropout=dropout_U,return_sequences=True))
		if args.dropout_prob > 0:
			model.add(Dropout(args.dropout_prob))
		...
		if args.rnn_dim > 0:
			forwards = RNN(args.rnn_dim, Dropout(dropout_W), return_sequences=False, recurrent_dropout=dropout_U)(output)
			backwards = RNN(args.rnn_dim, Dropout(dropout_W), return_sequences=False, recurrent_dropout=dropout_U, go_backwards=True)(output)
		if args.dropout_prob > 0:
		...
		if args.cnn_dim > 0:
			output = Conv1DWithMasking(nb_filter=args.cnn_dim, filter_length=args.cnn_window_size, border_mode=cnn_border_mode, subsample_length=1)(output)
		if args.rnn_dim > 0:
			forwards = RNN(args.rnn_dim,Dropout(dropout_W), return_sequences=True, reccurent_drouput=dropout_U)(output)
			backwards = RNN(args.rnn_dim, Dropout(dropout_W), return_sequences=True, reccurent_drouput=dropout_U, go_backwards=True)(output)
		if args.dropout_prob > 0:
			forwards = Dropout(args.dropout_prob)(forwards)
			backwards = Dropout(args.dropout_prob)(backwards)
